...
def save_pointmap(point_maps, output_dir):
    """
    Save the point map as a PLY file for visualization.
    point_maps: (N, 518, 518, 3) numpy array of 3D points
    output_dir: directory to save the PLY files
    """
    for idx in range(point_maps.shape[0]):
        points = point_maps[idx]  # (518, 518, 3)
        npy_path = os.path.join(output_dir, f"point_map_{idx}.npy")
        np.save(npy_path, points)

        # Flatten the point map to a list of points
        points = points.reshape(-1, 3)
        # Create a point cloud object using Open3D
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points)

        # Save the point cloud to a PLY file
        pointmap_path = os.path.join(output_dir, f"point_map_{idx}.ply")
