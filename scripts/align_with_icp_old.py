...
def calculate_icp_transformations(method="marching_cubes"):
    ...
    for view_idx, (view_data, vggt_surface) in enumerate([(view1_data, vggt_surface1), (view2_data, vggt_surface2)]):
        ...
        try:
            ...
            ## DEBUG: saved aligned surface point with VGGT
            combined_points = np.vstack([aligned_surface_points, vggt_surface])
            colors1 = np.tile([255, 0, 0], (len(aligned_surface_points), 1))  # Red
            colors2 = np.tile([0, 255, 0], (len(vggt_surface), 1))  # Green
            combined_colors = np.vstack([colors1, colors2]).astype(np.uint8)
            pc = trimesh.PointCloud(combined_points, colors=combined_colors)
            pc.export(f"DEBUG_VIEW_{view_idx + 1}.ply")
...
if __name__ == "__main__":
    ...
    # Debugging. Save the .ply files for views after ICP alignment with VGGT_surfaces
    view1_data
